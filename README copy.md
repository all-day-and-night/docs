# LLM 실습 노트 — VitePress 문서 사이트

AI Inference Platform 실습 내용을 정리한 VitePress 기반 문서 사이트입니다.

Local LLM 서빙부터 AWS EKS 배포, GitLab CI/CD, 성능 최적화, 비용 분석까지 실습 기반으로 정리합니다.
...
# 개발 서버 시작 (http://localhost:7001/docs/)
pnpm dev
```
...
├── index.md              # 홈 페이지
└── llm/
    ├── index.md          # AI Inference Platform 전체 실습 계획 (Phase 0~10)
    └── aws-infra/        # AWS 인프라 구축 단계별 가이드
        ...
        ├── 05-cicd-pipeline.md   # CI/CD 파이프라인 (.gitlab-ci.yml)
...
빌드/배포 파이프라인 확장 예정 → [CI/CD 파이프라인 가이드](docs/llm/aws-infra/05-cicd-pipeline.md)
...
```
cd existing_repo
git remote add origin http://localhost/developer1/docs.git
git branch -M main
...
* [Set up project integrations](http://localhost/developer1/docs/-/settings/integrations)
